...
def CNN_experiments(random_number,\
                    initial_Q_vec,\
                    n_feature,\
                    nneuron,\
    ...
    netnn = nn.Sequential(CNNlayer(n_feature,2,nneuron)) # build the black-box neural network
    ...
    for _ in range(numiter_nn):
        ...
        predicted_mlp=netnn(read_data_train_net.reshape((n_train+n_train_net,n_feature,1)))
        ...
        predicted_mlp_val=netnn(read_data_val.reshape((n_train+n_val,n_feature,1)))
    ...
    predicted_mlp_test=\
        netnn(read_data_test[b_ind.shape[0]+torch.arange(0,n_test),:].reshape((n_test,n_feature,1)))
